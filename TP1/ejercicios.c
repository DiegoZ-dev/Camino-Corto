...
void ej1()
{
    /* Dise�ar un programa que dados tres numeros enteros "dia", "mes", "a�o":
     *  Analice si la fecha que representan es v�lida, e indique el resultado por pantalla.
     */
    system("cls");
    int dia,mes,anio;
    int valido;
...
void ej3()
{
    /* Escribir un programa que permita llevar adelante las siguientes conversiones:
     ...
     Litros cada 100 km a millas por gal�n (1 litro = 0,264172052 galones).
    */
    system("cls");
...
void ej6()
{
    ...
    printf("%s en decimal es: %d",hex,hex2dec(hex));
}
void ej7()
{
...
int hex2dec(char *hex)
{
    ...
    while(*hex != '\0')
    {
        int digito = hexChar2dec(*hex);
        if(digito==-1)
        {
            printf("Error: El número no es hexadecimal.");
            exit(-1);
        }
        numero = numero * 16 + digito;
        hex++;
    }
    return numero;
}
